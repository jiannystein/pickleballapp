...
const DirectTestPage = () => {
  const [message, setMessage] = useState('Direct API test bug report');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Use the direct SQL API endpoint
      const response = await axios.post('/api/bug-reports/direct', {
        message,
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <h1 className="text-2xl font-bold mb-6">Direct SQL Bug Report Test</h1>
      ...
      <div className="max-w-md mx-auto bg-white p-6 rounded-lg shadow-lg">
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          >
            {loading ? 'Submitting...' : 'Submit Bug Report (Direct SQL)'}
          </button>
        </form>
